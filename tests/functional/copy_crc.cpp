...
#include <utils/copy_crc/actual.hpp>
#include <utils/copy_crc/reference.hpp>
#include <utils/copy_crc/workload_builder.hpp>
...
using types =
    std::tuple<std::uint32_t, std::uint32_t, std::uint32_t, reflection_e, data_reflection_e>;

class copy_crc: public ::testing::TestWithParam<types>
{
};

TEST_P(copy_crc, success)
{
    auto [transfer_size, src_alignment, dst_alignment, reflection, data_reflection] = GetParam();
