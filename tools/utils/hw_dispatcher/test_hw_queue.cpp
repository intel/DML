...
#define DML_TEST_HWSTS_RET(expr, err_code) \
    {                                      \
...
void hw_queue::set_portal_ptr(void* value_ptr) noexcept {
    portal_offset_ = reinterpret_cast<uint64_t>(value_ptr) & DML_TEST_OWN_PAGE_MASK;
    portal_mask_   = reinterpret_cast<uint64_t>(value_ptr) & (~DML_TEST_OWN_PAGE_MASK);
    portal_ptr_    = value_ptr;
}

auto hw_queue::get_portal_ptr() const noexcept -> void* {
    uint64_t offset = portal_offset_++;
    offset          = (offset << 6) & DML_TEST_OWN_PAGE_MASK;
    return reinterpret_cast<void*>(offset | portal_mask_);
}

auto hw_queue::initialize_new_queue(void* wq_descriptor_ptr) noexcept -> dml_test_hw_accelerator_status {

    auto* work_queue_ptr = reinterpret_cast<accfg_wq*>(wq_descriptor_ptr);
    char  path[64];

    if (ACCFG_WQ_ENABLED != dml_test_accfg_wq_get_state(work_queue_ptr)) {
        return DML_TEST_HW_ACCELERATOR_WORK_QUEUES_NOT_AVAILABLE;
    }

    if (ACCFG_WQ_SHARED != dml_test_accfg_wq_get_mode(work_queue_ptr)) {
        return DML_TEST_HW_ACCELERATOR_WORK_QUEUES_NOT_AVAILABLE;
    }

    auto status = dml_test_accfg_wq_get_user_dev_path(work_queue_ptr, path, 64 - 1);
    DML_TEST_HWSTS_RET((0 > status), DML_TEST_HW_ACCELERATOR_LIBACCEL_ERROR);

    auto fd = open(path, O_RDWR);
    if (0 > fd) { return DML_TEST_HW_ACCELERATOR_LIBACCEL_ERROR; }

    auto* region_ptr = mmap(nullptr, 0x1000U, PROT_WRITE, MAP_SHARED | MAP_POPULATE, fd, 0);
    close(fd);
    if (MAP_FAILED == region_ptr) { return DML_TEST_HW_ACCELERATOR_LIBACCEL_ERROR; }

    priority_ = dml_test_accfg_wq_get_priority(work_queue_ptr);
    size_     = dml_test_accfg_wq_get_size(work_queue_ptr);

    accfg_op_config op_cfg;
    const int32_t   get_op_cfg_status = dml_test_accfg_wq_get_op_config(work_queue_ptr, &op_cfg);
    if (get_op_cfg_status) {
        op_cfg_enabled_ = false;
    } else {
        for (uint8_t bit_group_index = 0; bit_group_index < DML_TEST_TOTAL_OP_CFG_BIT_GROUPS; bit_group_index++) {
