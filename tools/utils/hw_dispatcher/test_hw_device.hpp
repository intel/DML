...
#ifndef DML_TOOLS_UTILS_HW_DISPATCHER_TEST_HW_DEVICE_HPP_
#define DML_TOOLS_UTILS_HW_DISPATCHER_TEST_HW_DEVICE_HPP_
...
#define DML_TEST_DSA_DEVICE \
    ((uint32_t)(((uint32_t)0xFF << 24U) | ((uint32_t)('a') << 16U) | ((uint32_t)('s') << 8U) | (uint32_t)('d')))

/**< Max number of devices, used to initialize the devices array in hw_dispatcher */
#define DML_TEST_MAX_NUM_DEV 100U

/**< Character mask to convert the first 3 bytes to lower case and make the 4th 0xff */
#define DML_TEST_CHAR_MSK 0xFF202020

/**< IAACAP bit 0 - generation 2 Minimum Capabilities support */
#define DML_TEST_IC_GEN_2_MIN_CAP(IAACAP) (((IAACAP)) & 0x01)

/**< IAACAP bit 3 - Force Array Output Modification Support */
#define DML_TEST_IC_FORCE_ARRAY_OUTPUT_MOD(IAACAP) (((IAACAP) >> 3) & 0x01)

/**< IAACAP bit 7 - dictionary compression support */
#define DML_TEST_IC_DICT_COMP(IAACAP) (((IAACAP) >> 7) & 0x01)

/**< OPCFG bit {OP}  - generic operation support */
#define DML_TEST_OC_GET_OP_SUPPORTED(OPCFG, OP) ((((OPCFG)[7 - ((OP) / 32)]) >> (OP) % 32) & 0x01)

/**< Max number of workqueues, used to initialize the workqueue array in hw_dispatchwer */
#define DML_TEST_MAX_NUM_WQ 100U

class hw_device final {

    static constexpr uint32_t max_working_queues = DML_TEST_MAX_NUM_WQ;

    using queues_container_t   = std::array<hw_queue, max_working_queues>;
    using op_config_register_t = std::array<uint32_t, DML_TEST_TOTAL_OP_CFG_BIT_GROUPS>;
...
#endif //__linux__
#endif //DML_TOOLS_UTILS_HW_DISPATCHER_TEST_HW_DEVICE_HPP_
