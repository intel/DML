...
/**
 * @brief Contains an implementation of the @ref DML_OP_DIF_CHECK descriptor initialization
 * @date 6/4/2020
 *
 */
#include <assert.h>
...
typedef union
{
    uint32_t value;   /**< Contains 32-bit value */
    uint8_t  bytes[4]; /**< Contains 4-byte array */
} dif_flags_t;
...
DML_HW_BYTE_PACKED_STRUCTURE_BEGIN
{
    ...
    uint8_t                   reserved_1[8];               /**< Not used bytes in the descriptor */
    ...
    uint8_t                   reserved_2[2];               /**< Not used bytes in the descriptor */
    dif_flags_t               dif_flags;                   /**< Configuration of the DIF operation */
    uint8_t                   reserved_3[4];               /**< Not used bytes in the descriptor */
    ...
    uint8_t                   reserved_4[8];               /**< Not used bytes in the descriptor */
} own_check_dif_descriptor_t;
DML_HW_BYTE_PACKED_STRUCTURE_END

/*
 * Check that descriptor has a correct size
 */
static_assert(sizeof(own_check_dif_descriptor_t) == DSA_HW_DESCRIPTOR_SIZE,
                                                     ...
                                                     dsahw_completion_record_t *result_ptr)
{
    ...
    own_check_dif_descriptor_t *dif_descriptor_ptr = (own_check_dif_descriptor_t *) descriptor_ptr;

    // TODO Set privilege settings

    // Set operation_control
    dif_descriptor_ptr->operation_control.operation_type = DML_OP_DIF_CHECK;
    ...
    // Set dif properties
    dif_descriptor_ptr->dif_flags.value = dif_config_ptr->flags;

    // Reset reserved bits in the DIF flag fields
    dif_descriptor_ptr->dif_flags.bytes[OWN_DIF_DESTINATION_FLAGS] = 0u;
    dif_descriptor_ptr->dif_flags.bytes[OWN_DIF_RESERVED_FLAGS]    = 0u;
    dif_descriptor_ptr->dif_flags.bytes[OWN_DIF_GENERAL_FLAGS]    &= ~OWN_DIF_RESERVED_GENERAL_FLAGS;

    // Set DIF-block size
    dif_descriptor_ptr->dif_flags.bytes[OWN_DIF_GENERAL_FLAGS] |= dif_config_ptr->block_size;

    // Set tag seeds and application tag mask
    dif_descriptor_ptr->reference_tag_seed   = dif_config_ptr->source_reference_tag_seed;
    dif_descriptor_ptr->application_tag_seed = dif_config_ptr->source_application_tag_seed;
    dif_descriptor_ptr->application_tag_mask = dif_config_ptr->source_application_tag_mask;
