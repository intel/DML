...
/**
 * @brief Contains an implementation of the @ref DML_OP_DIF_STRIP descriptor initialization
 * @date 6/24/2020
 *
 */
#include <assert.h>
...
/**
 * @brief Represents descriptor for the @ref DML_OP_DIF_STRIP operation
 */
DML_HW_BYTE_PACKED_STRUCTURE_BEGIN
{
    ...
    uint8_t                   reserved_3[8];               /**< Not used bytes in the descriptor */
} own_strip_dif_descriptor_t;
DML_HW_BYTE_PACKED_STRUCTURE_END

/*
 * Check that descriptor has a correct size
 */
static_assert(sizeof(own_strip_dif_descriptor_t) == DSA_HW_DESCRIPTOR_SIZE,
                                                     ...
                                                     dsahw_completion_record_t *result_ptr)
{
    ...
    own_strip_dif_descriptor_t *dif_descriptor_ptr = (own_strip_dif_descriptor_t *) descriptor_ptr;

    // TODO Set privilege settings

    // Set operation_control
    dif_descriptor_ptr->operation_control.operation_type = DML_OP_DIF_STRIP;
    ...
    dif_descriptor_ptr->dif_flags.uint_8[OWN_DIF_DESTINATION_FLAGS] = 0u;
