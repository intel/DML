...
DML_HW_BYTE_PACKED_STRUCTURE_BEGIN
{
    ...
    uint8_t                   *memory_region_ptr;          /**< Pointer to the memory region */
 ...
 */
static_assert(sizeof(own_crc_descriptor_t) == DSA_HW_DESCRIPTOR_SIZE,
                                               ...
                                               dsahw_completion_record_t *result_ptr)
{
    ...
    own_crc_descriptor_t *op_descriptor_ptr = (own_crc_descriptor_t *)descriptor_ptr;
    ...
    op_descriptor_ptr->memory_region_ptr                          = source_ptr;
    op_descriptor_ptr->transfer_size                              = source_length;
    op_descriptor_ptr->operation_control.operation_type           = DML_OP_CRC;

    if (DML_FLAG_CRC_READ_SEED & flags)
    {
