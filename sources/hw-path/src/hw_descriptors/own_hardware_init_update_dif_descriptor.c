...
/**
 * @brief Contains an implementation of the @ref DML_OP_DIF_UPDATE descriptor initialization
 * @date 06/23/2020
 *
 */
#include <assert.h>
...
/**
 * @brief Represents descriptor for the @ref DML_OP_DIF_UPDATE operation
 */
DML_HW_BYTE_PACKED_STRUCTURE_BEGIN
{
    ...
    uint8_t                   reserved_2[4];                    /**< Not used bytes in the descriptor */
    uint32_t                  source_reference_tag_seed;        /**< Seed for reference tag of the source */
    uint16_t                  source_application_tag_mask;      /**< Mask for application tag of the source */
    uint16_t                  source_application_tag_seed;      /**< Seed for application tag of the source */
    uint32_t                  destination_reference_tag_seed;   /**< Seed for reference tag of the destination */
    uint16_t                  destination_application_tag_mask; /**< Mask for application tag of the destination */
    uint16_t                  destination_application_tag_seed; /**< Seed for application tag of the destination */
} own_update_dif_descriptor_t;
DML_HW_BYTE_PACKED_STRUCTURE_END

/*
 * Check that descriptor has a correct size
 */
static_assert(sizeof(own_update_dif_descriptor_t) == DSA_HW_DESCRIPTOR_SIZE,
                                                      ...
                                                      dsahw_completion_record_t *result_ptr)
{
    ...
    OWN_CHECK_PROTECTED_STREAM_SIZE(source_length,
    ...
    own_update_dif_descriptor_t *dif_descriptor_ptr = (own_update_dif_descriptor_t *) descriptor_ptr;

    // TODO Set privilege settings

    // Set operation_control
    dif_descriptor_ptr->operation_control.operation_type = DML_OP_DIF_UPDATE;
    ...
    // Set tag seeds and application tag mask for source
    dif_descriptor_ptr->source_reference_tag_seed   = dif_config_ptr->source_reference_tag_seed;
    dif_descriptor_ptr->source_application_tag_seed = dif_config_ptr->source_application_tag_seed;
    dif_descriptor_ptr->source_application_tag_mask = dif_config_ptr->source_application_tag_mask;

    // Set tag seeds and application tag mask for destination
    dif_descriptor_ptr->destination_reference_tag_seed   = dif_config_ptr->destination_reference_tag_seed;
    dif_descriptor_ptr->destination_application_tag_seed = dif_config_ptr->destination_application_tag_seed;
    dif_descriptor_ptr->destination_application_tag_mask = dif_config_ptr->destination_application_tag_mask;
