...
/**
 * @brief Contains an implementation of the @ref DML_OP_COMPARE_PATTERN descriptor initialization
 * @date 5/21/2020
 *
 */
#include <assert.h>
...
/**
 * @brief Represents descriptor for the @ref DML_OP_COMPARE_PATTERN operation
 */
DML_HW_BYTE_PACKED_STRUCTURE_BEGIN
{
    ...
    uint8_t                   pattern[8];                  /**< 8-byte pattern */
...
} own_compare_pattern_descriptor_t;
DML_HW_BYTE_PACKED_STRUCTURE_END

/*
 * Check that descriptor has a correct size
 */
static_assert(sizeof(own_compare_pattern_descriptor_t) == DSA_HW_DESCRIPTOR_SIZE,
                                                           ...
                                                           dsahw_completion_record_t *result_ptr)
{
    ...
    own_compare_pattern_descriptor_t *op_descriptor_ptr = (own_compare_pattern_descriptor_t *) descriptor_ptr;

    // TODO Set privilege settings

    // Set operation_control
    op_descriptor_ptr->operation_control.operation_type = DML_OP_COMPARE_PATTERN;
    ...
    op_descriptor_ptr->source_first_ptr  = source_ptr;
    op_descriptor_ptr->transfer_size     = source_length;

    // Set pattern
    uint64_t *const descriptor_pattern_ptr = (uint64_t *) &op_descriptor_ptr->pattern[0];
    uint64_t *const job_pattern_ptr        = (uint64_t *) &pattern_ptr[0];

    *descriptor_pattern_ptr = *job_pattern_ptr;
