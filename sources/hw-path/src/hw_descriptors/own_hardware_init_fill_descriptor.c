...
/**
 * @brief Represents descriptor for fill operation
 */
DML_HW_BYTE_PACKED_STRUCTURE_BEGIN
{
    ...
    uint64_t                  pattern;                     /**< Pattern to fill the destination                                               */
    uint8_t                   *destination_ptr;            /**< Pointer to the destination to fill with the pattern                           */
...
} own_fill_descriptor_t;
DML_HW_BYTE_PACKED_STRUCTURE_END

/*
 * Check that descriptor has a correct size
 */
static_assert(sizeof(own_fill_descriptor_t) == DSA_HW_DESCRIPTOR_SIZE,
                                                ...
                                                dsahw_completion_record_t *result_ptr)
{
    ...
    own_fill_descriptor_t *op_descriptor_ptr = (own_fill_descriptor_t *) descriptor_ptr;

    // TODO Set privilege settings

    // Set operation_control
    op_descriptor_ptr->operation_control.operation_type = DML_OP_FILL;
    ...
    op_descriptor_ptr->pattern         = *pattern_ptr;
